...
<div class="s1-assignment">
  ...
  <div style="background:rgba(255,255,255,0.06);border:1px solid rgba(255,193,7,0.25);border-radius:0.6rem;padding:1rem 1.15rem;margin-bottom:1.25rem;">
    ...
    <ol style="margin:0;padding-left:1.25rem;color:#B0BEC5;font-size:0.8rem;line-height:1.7;">
      <li><strong style="color:#CFD8DC;">Dataset loading will break</strong> if you use <code style="background:rgba(255,255,255,0.08);padding:1px 4px;border-radius:3px;font-size:0.75rem;">load_dataset("ncbi_disease")</code> or <code style="background:rgba(255,255,255,0.08);padding:1px 4px;border-radius:3px;font-size:0.75rem;">trust_remote_code=True</code>. The legacy loading scripts are no longer supported. Load from HuggingFace&rsquo;s auto-converted Parquet files instead: use <code style="background:rgba(255,255,255,0.08);padding:1px 4px;border-radius:3px;font-size:0.75rem;">load_dataset("parquet", data_files={...})</code> with URLs pointing to <code style="background:rgba(255,255,255,0.08);padding:1px 4px;border-radius:3px;font-size:0.75rem;">refs%2Fconvert%2Fparquet</code>.</li>
      <li><strong style="color:#CFD8DC;">Evaluate before you train.</strong> Run <code style="background:rgba(255,255,255,0.08);padding:1px 4px;border-radius:3px;font-size:0.75rem;">trainer.evaluate()</code> on the test set <em>before</em> calling <code style="background:rgba(255,255,255,0.08);padding:1px 4px;border-radius:3px;font-size:0.75rem;">trainer.train()</code>. This captures the random-head baseline so you can show a before/after delta table.</li>
      <li><strong style="color:#CFD8DC;">Label names aren&rsquo;t in the Parquet.</strong> The auto-converted files store <code style="background:rgba(255,255,255,0.08);padding:1px 4px;border-radius:3px;font-size:0.75rem;">ner_tags</code> as plain integers. Define <code style="background:rgba(255,255,255,0.08);padding:1px 4px;border-radius:3px;font-size:0.75rem;">LABEL_NAMES = ["O", "B-Disease", "I-Disease"]</code> yourself &mdash; don&rsquo;t rely on <code style="background:rgba(255,255,255,0.08);padding:1px 4px;border-radius:3px;font-size:0.75rem;">ClassLabel</code> metadata.</li>
      <li><strong style="color:#CFD8DC;">Switch runtime first.</strong> Go to <em>Runtime &rarr; Change runtime type &rarr; T4 GPU</em> before running anything. Training on CPU will take 10&times; longer.</li>
      <li><strong style="color:#CFD8DC;">Sub-token alignment matters.</strong> BERT splits words like &ldquo;nephroblastoma&rdquo; into multiple sub-tokens. The first sub-token gets the NER label; continuations get <code style="background:rgba(255,255,255,0.08);padding:1px 4px;border-radius:3px;font-size:0.75rem;">-100</code> so the loss ignores them.</li>
      <li><strong style="color:#CFD8DC;">Use seqeval strict mode</strong> for entity-level F1 &mdash; not token-level accuracy. Token accuracy (~98%) is misleading because most tokens are &ldquo;O&rdquo;.</li>
    </ol>
  </div>
